      ...
      {showHeader ? (
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-base leading-relaxed text-muted sm:text-lg">
            Preços em MZN/mês. Comece com 60 dias de trial, sem cartão
      ...
      >
        {plans.map((plan, i) => (
          <Reveal
            key={plan.id}
            delay={i * 100}
            variant={plan.highlighted ? "scale" : "fade-up"}
          >
            <article
              className={`relative flex h-full flex-col rounded-[1.5rem] p-6 transition duration-300 sm:p-7 ${
                plan.highlighted
                  ? "bg-strawberry text-white shadow-[0_16px_40px_rgba(230,62,92,0.28)] hover:-translate-y-1 hover:shadow-[0_20px_48px_rgba(230,62,92,0.35)]"
                  : "card lift"
              }`}
            >
              {plan.highlighted ? (
              ...
              <a
                ...
                className={`mt-8 inline-flex min-h-11 items-center justify-center rounded-full px-5 text-sm font-semibold transition ${
                  plan.highlighted
                    ? "bg-white text-strawberry hover:bg-white/95 hover:-translate-y-0.5"
                    : "bg-strawberry text-white hover:brightness-95 hover:-translate-y-0.5"
                }`}
              >
